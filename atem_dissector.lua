...
pf_fields["pf_cmd_mpfe_typeseenote"]    = ProtoField.new  ("Type (see note)", "atem.cmd.mpfe.typeseenote", ftypes.UINT8, nil, base.DEC)
...
function atem_proto.dissector(tvbuf,pktinfo,root)
	...
	while (pktlen_remaining > 0) do
		...
		elseif (cmd_name == "MPfe") then
			cmd_tree:add(pf_fields["pf_cmd_mpfe_typeseenote"], tvbuf:range(pos+8, 1))
			cmd_tree:add(pf_fields["pf_field_unknown1"], tvbuf:range(pos+9, 1))
